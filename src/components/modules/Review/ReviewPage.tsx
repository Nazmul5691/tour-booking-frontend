...
//       // Filter only COMPLETE bookings without reviews
...
// Import the IBooking type if exported, otherwise we use the corrected local interface

// 1. Updated interface to allow undefined createdAt to match IBooking
interface Booking {
  _id: string;
  tour: {
    _id: string;
  ...
  status: string;
  guestCount: number;
  createdAt?: string; // Changed to optional to fix the TS error
  hasReview?: boolean;
}
...
export default function Review() {
  ...
  const formatDate = (dateString?: string) => {
    if (!dateString) return "N/A"; // Handle the undefined case safely
    return new Date(dateString).toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
