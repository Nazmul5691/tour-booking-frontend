...
export const buildSystemPrompt = (tours: any[]) => {
    const tourList = tours
        .map(
            (t, i) => `${i + 1}. ${t.title} | ${t.location} | ৳${t.costFrom?.toLocaleString()} | ${t.slug}`
        )
        .join("\n");

    return `You are a Dream Tour AI assistant for Bangladesh tours.
...
- Use tour data to answer questions
- For booking: visit /allTours/tours/[slug]
