...
import { forgotPasswordReset } from "@/services/auth/auth.service";
import { useSearchParams } from "next/navigation";
import { useActionState, useEffect } from "react";
import { toast } from "sonner";

const ForgotPasswordResetForm = ({ redirect }: { redirect?: string }) => {
    //   const [state, formAction, isPending] = useActionState(forgetPassword, null);
    const searchParams = useSearchParams();
    const id = searchParams.get("id");
    const token = searchParams.get("token");

    const [state, formAction, isPending] = useActionState(forgotPasswordReset, null);
